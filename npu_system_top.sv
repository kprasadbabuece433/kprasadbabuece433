...
//    0x200..0x5FC  DMEM window (256 × 32-bit data words)
//
//  UART protocol: handled by uart_apb_sys / uart_apb_master.
//    Lock address : 0xFFFFFFF0, key : 0xDEAD10CC
//    Default baud : clk / (DEFAULT_DIVISOR+1)
//
//  Parameters:
//    CLK_FREQ_HZ     : input clock frequency (used for baud comment only)
//    DEFAULT_DIVISOR : baud rate divisor (e.g. 87 @ 8 MHz → ~115200 baud)
...
module npu_system_top #(
    ...
    parameter DEFAULT_DIVISOR = 16'd87,       // ~115200 @ 8 MHz
    ...
    // ── Status ────────────────────────────────────────────────
    output wire locked,           // APB bus locked (from uart_apb_sys)
    output wire npu_done,         // NPU reached HALT
    output wire done_processing   // alias for above (all instr done)
);

// ================================================================
//  APB splitter → Slave 0 wires (npu_apb_decoder)
// ================================================================
...
// ================================================================
//  Unused slaves 1-7: tie PRDATA=0, PREADY=1, PSLVERR=0
// ================================================================
